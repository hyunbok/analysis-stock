...
- 종료: 에이전트 정리 (아래 "에이전트 종료 프로토콜" 참조) → `TeamDelete`
...
1. 완료 메시지 수신 → `TaskUpdate(completed)` + `shutdown_request` 즉시 전송
2. `teammate_terminated` 확인까지 추적. 미응답 시 1회 재시도 후 넘어감
3. **다음 phase 스폰 전** 이전 phase 에이전트 terminated 확인
4. `TeamDelete` 전 모든 에이전트 terminated 확인 (미종료 시 TeamDelete 실패 가능)
5. idle ≠ terminated. shutdown 후 idle만 오면 재시도 필요
